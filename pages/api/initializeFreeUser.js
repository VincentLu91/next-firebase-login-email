    ...
    const { error } = await supabase.from("customers").insert([
      {
        id: id,
        email_address: email,
        mic_tokens: 2000, // otherwise 2000, but should be no credits - indie hacking
        call_tokens: 6, // otherwise 2000, but should be no credits - indie hacking
        num_calls: 6, // otherwise 2000, but should be no credits - indie hacking
